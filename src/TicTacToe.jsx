...
let data = ["","","","","","","","",""]; //storage of tic-tac-toe game

const TicTacToe = () => {

    let [count, setCount] = useState(0);
    let [lock, setLock] = useState(false);
    let titleRef = useRef(null); //to show winner 
    ...
    const toggle = (e, num) => {
        if(lock) {
            return 0; //if user wins toggle function won't work
        }
        if(count%2===0) {
    ...
    const reset = () =>{
        setLock(false);
        data = ["","","","","","","","",""]; //storage of tic-tac-toe game
